...
/**
 * MACD 死叉：DIF 下穿 DEA，即 hist < 0
 * threshold 字段在边沿触发中未使用（事件型触发，effective threshold=0）
 * 重置：hist >= 0（即死叉消失/反向） 或 经过 reArmHours 小时
 */
@Component
public class MacdDeathCrossEvaluator implements AlertEvaluator {
    @Override
    public String supportedType() { return "macd_death_cross"; }
    ...
    public EvaluationResult evaluate(Alert alert, RefreshedPrice data) {
        ...
        boolean fire = ArmedResetPolicy.check(
                alert, hist, ArmedResetPolicy.Direction.BELOW, LocalDateTime.now());
