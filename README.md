...
- **LightGBM 三窗口预测**：60d / 120d / 全量历史，三个时间尺度投票
- **MiniTransformer**：自注意力时序预测（numpy 手写，轻量、无外部 DL 框架依赖）
- **DQN 强化学习**：直接学"买/卖/持有"策略
- **技术信号打分**：RSI / MACD / 均线 / 布林带 综合评分
- **回测验证**：夏普比率、最大回撤、胜率、盈亏比，**让模型自己交账**

模型自动 6 小时重训，结果落盘（`models/*.pkl`），重启不丢。
