...
**P-2** | **日期区间回测**：`/strategies/backtest` 支持区间；`get_history` 真正使用 `start_date/end_date` | 现在 `get_history` 恒取最近 500 根、区间参数未被使用（`akshare_client.py:284,297`）；§4.D2 的"多时段裁决"与 §4.D1 ④ 都依赖它 |
